...
describe('api contract', () => {
    ...
    test('surface condition: uses unversioned routes and returns raw arrays', async () => {
        ...
        fetchMock.mockResolvedValueOnce(jsonResponse({ intentionId: 'int-1', paymobOrderId: 123, clientSecret: 'cs', checkoutUrl: 'https://accept.paymob.com/unifiedcheckout/?publicKey=pub&clientSecret=cs' }));
        const payment = await api.createPayment('o1');
        expect(payment.checkoutUrl).toContain('clientSecret=cs');
        const [url, init] = fetchMock.mock.calls[1];
        expect(url).toBe('/api/payments/orders/o1');
